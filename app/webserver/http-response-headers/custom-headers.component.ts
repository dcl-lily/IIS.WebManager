...
@Component({
    ...
    template: `
        <button class="create" (click)="create()" [disabled]="locked" [class.inactive]="_editing != -1"><i class="fa fa-plus color-active"></i><span>添加</span></button>
        <div class="container-fluid">
            <div class="row hidden-xs border-active grid-list-header" [hidden]="headers.length == 0">
                <label class="col-sm-4 col-lg-5">名称</label>
        ...
        <ul class="grid-list container-fluid">
            <li *ngFor="let header of headers; let i = index;">
                <div class="row grid-item" [class.background-editing]="_editing == i">
                    <div class="actions">
                        <button class="no-border" title="确认" [disabled]="locked || !isValid(header) || null" *ngIf="_editing == i" (click)="onFinishEditing(i)">
                        ...
                        <button class="no-border" *ngIf="header.id" title="删除" [disabled]="locked || _editing == i" [class.inactive]="_editing !== -1 && _editing !== i" (click)="onDelete(i)">
                            <i class="fa fa-trash-o red"></i>
                        </button>
                    </div>
                    <fieldset class="col-xs-8 col-sm-4 col-lg-5">
                        <label class="visible-xs">名称</label>
                        <label *ngIf="_editing == i" class="hidden-xs">名称</label>
