        ...
        <div *ngIf="_current" tabindex="-1" class="wrapper" 
            ...
            <div #header class="container-fluid hidden-xs">
                <div class="border-active grid-list-header row">
                    ...
                    <label class="col-sm-3 col-md-2 hidden-xs" [ngClass]="_orderBy.css('last_modified')" (click)="sort('last_modified')">修后修该时间</label>
                    <label class="col-md-2 visible-lg visible-md" [ngClass]="_orderBy.css('description')" (click)="sort('description')">类型</label>
                    <label class="col-md-1 visible-lg visible-md text-right" [ngClass]="_orderBy.css('size')" (click)="sort('size')">大小</label>
                </div>
            </div>
...
export class FileListComponent implements OnInit, OnDestroy {
    ...
    public deleteFiles(e: Event, files: Array<ApiFile>) {
        ...
        let msg = files.length == 1 ? "你确认要删除'" + files[0].name + "?" :
            "你确认要删除" + files.length + " items?";
    ...
    private deleteLocations(files: Array<ApiFile>) {
        ...
        let msg = files.length == 1 ? "你确定要移除根目录 '" + files[0].name + "'?" :
            "你确定要移除 " + files.length + "根目录?";
